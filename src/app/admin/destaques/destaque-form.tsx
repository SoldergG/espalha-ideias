  ...
  return (
    <form action={formAction} className="flex flex-col gap-5 border border-border bg-paper p-6">
      ...
      <div>
        ...
        <textarea name="resumo" defaultValue={initialValues.resumo} rows={3} className={inputClass} />
      </div>
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
        <div>
          ...
          <input
            name="data_destaque"
            type="date"
            defaultValue={initialValues.data_destaque ?? ""}
            className={inputClass}
          />
        </div>
        <div>
          <label className="block text-sm font-medium text-ink">Link (opcional)</label>
          <input name="link_href" defaultValue={initialValues.link_href ?? ""} className={inputClass} />
        </div>
      </div>
